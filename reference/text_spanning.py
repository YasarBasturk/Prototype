...
def extract_text_items(ocr_data):
    ...
    # For the PP-StructureV2 output which has a nested structure
    if 'res' in ocr_data and isinstance(ocr_data['res'], list):
        for item in ocr_data['res']:
            if isinstance(item, dict) and 'res' in item and isinstance(item['res'], list):
                return item['res']  # Return the nested text items
        return ocr_data['res']  # Return the top-level items
...
def process_document(cell_json_path, ocr_json_path, output_dir="combined_results", 
    ...
    try:
        ...
        if image_path and os.path.exists(image_path):
            ...
            create_visualization_with_spanning(cell_data, ocr_data, merged_data, vis_path, image_path)
    except ImportError:
